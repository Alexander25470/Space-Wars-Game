        ...
        for(auto enemy:enemies)
        {
            if(enemy->isShooting())
            {
                ...
                for(int i=-1;i<=1;i++)
                {
                    ...
                    f->initializer(width,height,enemyBulletAnim,enemy->getPos().x,enemy->getPos().y,enemy->getAngle()+(i*3),10);
...
void Game::eventListener() {
        ...
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::Up))
        {
            ...
            ship->getAnim()=shipAnim_M;
        }else
        {
            (ship->advanceShip(false),ship->getAnim()=shipAnim);
        }
